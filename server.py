...
def transition_to_next_map(state):
    ...
    # 偶数图（2/4/6）复用上一张图
    if next_map % 2 == 0 and next_map > 1:
        map_data, connections, grid_size = generate_map(next_map, state["map_data"], state["connections"])
    else:
...
# ==================== 前端页面 ====================
from _embedded_html import get_game_html
